...
class Grid(Model):
    ...
    game_types = {"Rock-Paper-Scissors": "RPS", "Prisoner's Dilemma": "PD"}
    ...
    def __init__(self, height=50, width=50, schedule_type="Random", game_type="Rock-Paper-Scissors"):
        '''
        ...
        self.game_type = self.game_types[game_type]
        ...
        if self.game_type == "PD":
            self.datacollector = DataCollector(
                {"Cooperating": lambda m: len([a for a in m.schedule.agents if a.move == "C"]),
                 "Defecting": lambda m: len([a for a in m.schedule.agents if a.move == "D"])})
